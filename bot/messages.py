...
def time_departure(place_of_arrival, departure_place, arrival_date, time, seat) -> str:
    departure_place = departure_place.replace('-','')
    return  f'Отлично! Теперь я сообщу вам, когда появится билет {code.cities.get(departure_place)} - {code.cities.get(place_of_arrival)}\nПолка: {seat}\n{arrival_date}: {time}' 
